...
from amberelectric import AmberApi as FlowPowerApi, ApiClient, Configuration, ChannelType, SpikeStatus
...
class FlowPowerClient:
    def __init__(self, db: Database):
        self.db = db
        configuration = Configuration(access_token=config.flowpower.api_key)
        api_client = ApiClient(configuration=configuration)
        self.api = FlowPowerApi(api_client=api_client)
        self.site_id = config.flowpower.site_id or self._discover_site_id()

    def _discover_site_id(self) -> str:
        sites = self.api.get_sites()
        if not sites:
            raise RuntimeError("No Flow Power sites found for this API key")
        site_id = sites[0].id
        logger.info("Auto-discovered Flow Power site: %s", site_id)
        return site_id
